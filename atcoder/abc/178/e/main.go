...
var (
	n                      int
	x, y                   [2 * 10e5]int
	maxA, maxB, maxC, maxD int = -1e12, -1e12, -1e12, -1e12
)

func main() {
	readVariables()
	var answer int = -1e12
	answer = MaxInt(maxA+maxD, answer)
	answer = MaxInt(maxB+maxC, answer)
	fmt.Println(answer)
}
...
func readVariables() {
	...
	for i := 0; i < n; i++ {
		x[i] = nextInt()
		y[i] = nextInt()
		maxA = MaxInt(maxA, x[i]+y[i])
		maxD = MaxInt(maxD, -x[i]-y[i])
		maxB = MaxInt(maxB, x[i]-y[i])
		maxC = MaxInt(maxC, -x[i]+y[i])
	}
}
